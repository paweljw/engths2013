	...
	public:
		...
		void solve()
		{
			...
			*/
			...
			backend->arg(Mangler, 4, gpu_flops);
			...
			*/
			...
			backend->arg(Resolver, 1, gpu_rhs_handle);
			backend->arg(Resolver, 2, gpu_map_handle);
			backend->arg(Resolver, 3, gpu_N_handle);
			backend->arg(Resolver, 4, gpu_blocknum_handle);
			backend->arg(Resolver, 6, gpu_flops);

			// This will store 5th parameter CPU-side
		...
		void CompileKernels()
		{
			...
			mykernels = util::replaceKernelTag(mykernels, "--TAG_LOCAL_MAP_SIZE--", N);
